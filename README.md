...
布隆过滤器也能给图片等多模态内容做“指纹存在性过滤”。

### 场景：内容平台图片去重

用户上传图片时，平台希望快速判断“这张图片是否已经见过”。直接存原图或完整特征很占空间，所以通常会先计算一个**感知哈希指纹**（perceptual hash），再用布隆过滤器保存这些指纹：
...
- 这是**指纹存在性过滤**，不是“以图搜图”或语义相似检索。
- 感知哈希对缩放、压缩、轻微调色有一定鲁棒性，但两张完全不同的图片仍可能产生相同指纹（哈希冲突）或让布隆过滤器误判。
- 布隆过滤器的“可能见过”结果必须再用精确存储确认。
...
完整可运行代码见：[demos/multimodal_image_dedup_demo.py](./demos/multimodal_image_dedup_demo.py)

这个 demo 会：

1. 用 Pillow 生成若干合成测试图片。
2. 为每张图片计算一个 256-bit 的简化感知哈希指纹。
3. 把指纹加入布隆过滤器，并把指纹保存到内存中的精确存储。
4. 查询原图、经过缩放和 JPEG 重编码的变体（应判定为重复）。
5. 查询全新的图片（应判定为新图片）。
